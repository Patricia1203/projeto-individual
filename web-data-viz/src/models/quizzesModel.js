...
function salvarResultado(id_usuario, id_quiz, pontos) {
    ...
    var buscarTentativaSql = `
        SELECT IFNULL(MAX(tentativa), 0) AS ultimaTentativa FROM resposta_usuario WHERE id_usuario = ${id_usuario} AND id_quiz = ${id_quiz};
    `;
    ...
    return database.executar(buscarTentativaSql).then(resultado => {
        ...
        var inserirSql = `
            INSERT INTO resposta_usuario (id_usuario, id_quiz, tentativa, pontos)   VALUES (${id_usuario}, ${id_quiz}, ${novaTentativa}, ${pontos});
        `;
